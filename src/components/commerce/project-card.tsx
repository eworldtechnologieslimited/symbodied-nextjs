...
export function ProjectCard({ name, category, summary, raised, target, creator, daysLeft, seed = 0, image, dark }: ProjectCardProps) {
  ...
  const cardClass = dark
    ...
    : "bg-white dark:bg-[#162018] border-ink-200 dark:border-[#263a2b] text-ink dark:text-[#dceee3]";
  return (
    <div className={`rounded-xl border shadow-[var(--shadow-sm)] overflow-hidden transition-all duration-200 hover:shadow-[var(--shadow-lg)] hover:-translate-y-0.5 ${cardClass}`}>
      ...
      <div className="p-5 flex flex-col gap-3">
        ...
        <h3 className={`font-sans font-bold text-base leading-snug line-clamp-1 ${dark ? "text-white" : "text-ink dark:text-[#dceee3]"}`}>{name}</h3>
        <p className={`text-sm leading-relaxed line-clamp-2 ${dark ? "text-white/70" : "text-ink-600 dark:text-[#89a895]"}`}>{summary}</p>
        <div className="flex flex-col gap-1.5">
          <div className={`flex justify-between text-xs font-sans ${dark ? "text-white/60" : "text-ink-500 dark:text-[#668074]"}`}>
            <span className={`font-semibold ${dark ? "text-white" : "text-ink dark:text-[#dceee3]"}`}>{naira(raised)} raised</span>
            <span>{funded}%</span>
          </div>
