...
export default function StudioCreateEventPage() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      else router.push("/studio/events");
    });
  };
    ...
    <div className="p-7 max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-ink-200 p-6 flex flex-col gap-5">
        <div className="flex flex-col gap-1.5">
          ...
          <input name="name" required placeholder="e.g. Community Harvest Festival" className={field} />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <input name="venue" placeholder="e.g. Community Hall" className={field} />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className={label}>Location / City</label>
            <input name="location" placeholder="e.g. Enugu" className={field} />
          </div>
        </div>
        <div className="flex flex-col gap-1.5">
        ...
        <p className="text-xs text-ink-400 font-sans">Your event will appear on the events page immediately.</p>
        <div className="flex justify-end gap-3">
          <Button type="button" variant="ghost" onClick={() => router.push("/studio/events")}>Cancel</Button>
